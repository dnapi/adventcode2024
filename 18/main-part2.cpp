...
long min_score(std::vector<std::string> data, std::vector<std::vector<long>>& scores
    , State s)
    {
    if (scores[data.size() - 2][data.size() - 2] != MAX_SCORE)
        return scores[data.size() - 2][data.size() - 2];
    if (data[s.y][s.x] == 'E'){
...
int main() {
    ...
    size_t size = 71;
    size_t num_bytes_old = 1024;

    // std::ifstream file("input-test.txt");
    // size_t size = 7;
    // size_t num_bytes_old = 1;
    // size_t num_bytes = 12;
    ...
    size_t num_bytes_new = num_bytes_old + (num_bytes - num_bytes_old) / 2;
    size_t score = 0;

    while (num_bytes_new != num_bytes_old){
        std::cout << "num_bytes_new=" << num_bytes_new << std::endl;
        std::vector<std::string> map(size + 2, std::string(size + 2, '.'));
        for (size_t i = 0; i < num_bytes_new; i++){
        ...
        score = scores[size][size];
        std::cout << "score = "  << score << std::endl;
        if (score == MAX_SCORE){
            num_bytes_new = num_bytes_old + (num_bytes_new - num_bytes_old) / 2;
        }
        else {
            num_bytes_old = num_bytes_new;
            num_bytes_new = num_bytes_new + (num_bytes - num_bytes_new) / 2;
        }
    }
    std::cout << "score = "  << score << std::endl;
    std::cout << "num_bytes_new = "  << num_bytes_new << std::endl;
    std::cout << "result= " << data[num_bytes_new][0] << "," << data[num_bytes_new][1] << std::endl;
